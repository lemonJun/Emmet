...
public class EliminationArray implements IEliminationArray {
    ...
    public boolean tryAdd(Object obj, int backOff) throws InterruptedException {
        ...
        for (int i = 0; i < lookahead; i++) {
            ...
            if (removeObj == TOMB_STONE && removeList.compareAndSet(index, TOMB_STONE, obj) && DEBUG) {
    ...
    public Object tryRemove(int backOff) throws InterruptedException {
        ...
        for (int i = 0; i < lookahead; i++) {
            ...
            if (objAdd != null && objAdd != REMOVED && addLlist.compareAndSet(index, objAdd, REMOVED)) {
                if (DEBUG)
                    succ.incrementAndGet();
                return objAdd;
            }
        ...
        for (int i = 0; i < lookahead; i++) {
            ...
            if (oldRemove == null && removeList.compareAndSet(index, null, TOMB_STONE)) {
                Thread.sleep(backOff);
                while (true) {
