...
public class LockFreeSet<E> implements Set<E> {
    ...
    private void initInternal() {
         ...
         */
        AtomicReferenceArray<AtomicMarkableReference<HashLinkNode<E>>> segment = new AtomicReferenceArray<AtomicMarkableReference<HashLinkNode<E>>>(segmentSize);
    ...
    private AtomicMarkableReference<HashLinkNode<E>> bucketAt(int hash) {
        ...
        AtomicReferenceArray<AtomicMarkableReference<HashLinkNode<E>>> seg = segments.get(segment);
        if (seg != null) {
        ...
        if (seg == null) {
            seg = new AtomicReferenceArray<AtomicMarkableReference<HashLinkNode<E>>>(segmentSize);
    ...
    public boolean add(E e) {
        ...
        if (addElement(e, bucket, hash)) {
            ...
            if (totalElement.get() > cSize * loadFactor && cSize < N_SEGMENT * segmentSize)
    ...
    private CompositeStateHolder<?> findNode(HashLinkNode<E> o, AtomicMarkableReference<HashLinkNode<E>> start, CompositeStateHolder<E> holder) {
        try_again: while (true) {
            ...
            while (true) {
                ...
                if (holder.prev.getReference() != cur2 || holder.prev.isMarked()) {
                    continue try_again;
                }
                if (!cur2.next.isMarked()) {
                    ...
                    if (!holder.prev.compareAndSet(cur2, holder.next, false, false)) {
                        continue try_again;
                    }
                }
     ...
     */
    private CompositeStateHolder<?> findElement(E o, AtomicMarkableReference<HashLinkNode<E>> start, CompositeStateHolder<E> holder, int hash) {
        ...
        try_again: while (true) {
            ...
            while (true) {
                ...
                if (!nextprt.isMarked()) {
                    ...
                    if (!prev.compareAndSet(cur, nextprt.getReference(), false, false)) {
                        continue try_again;
                    }
                }
     ...
     */
    boolean findElement(E o, AtomicMarkableReference<HashLinkNode<E>> start, int hash) {
     ...
     */
    HashLinkNode<E> insert(HashLinkNode<E> e, AtomicMarkableReference<HashLinkNode<E>> start) {
     ...
     */
    boolean addElement(E e, AtomicMarkableReference<HashLinkNode<E>> start, int hash) {
        CompositeStateHolder<E> holder = new CompositeStateHolder<E>();
        ...
        while (true) {
            ...
            if (node == null) {
                ...
                node.next = new AtomicMarkableReference<HashLinkNode<E>>(cur2, false);
            } else
     ...
     */
    boolean removeElement(E o, AtomicMarkableReference<HashLinkNode<E>> start, int hash) {
        CompositeStateHolder<E> holder = new CompositeStateHolder<E>();
        while (true) {
