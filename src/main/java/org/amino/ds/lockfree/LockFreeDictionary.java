...
public class LockFreeDictionary<K, V> implements Map<K, V> {
    ...
    private static class Node<K, V> {
        ...
        public Node() {
            ...
            this.prev = null;
            this.next = new ArrayList<AtomicMarkableReference<Node<K, V>>>(MAXLEVEL);
            this.version = 0;
            this.key = null;
    ...
    private Node<K, V> helpDelete(Node<K, V> n, int ll) {
        ...
        while (true) {
            ...
            if (prev.next.get(ll).compareAndSet(n, n.next.get(ll).getReference(), false, false)) {
    ...
    private Node<K, V> searchLevel(Node<K, V> last, int ll, K k) {
        ...
        while (true) {
            ...
            if (n2 == null) {
                ...
                if (((n1.validLevel > ll) || (n1 == last) || (n1 == stop)) && (compare(n1.key, k) < 0) && (compare(n1.key, last.key) >= 0)) {
                    if (n1.validLevel <= ll) {
                        n1 = last;
    ...
    private NodeData<V> insert(K k, V d) {
        ...
        while (true) {
            ...
            if ((!n2.data.isMarked()) && (n2.data.getReference() != null) && (compare(n2.key, k) == 0)) {
            ...
            if (kk == 0) {
                newNode.next.add(new AtomicMarkableReference<Node<K, V>>(n2, false));
                kk++;
            } else {
                newNode.next.set(0, new AtomicMarkableReference<Node<K, V>>(n2, false));
            }
        ...
        for (i = 1; i < level - 1; i++) {
            ...
            while (true) {
                ...
                if (kk == 0) {
                    ...
                    newNode.next.set(i, new AtomicMarkableReference<Node<K, V>>(n2, false));
                }
    ...
    private V delete(K k, boolean delVal, V dd) {
        ...
        while (true) {
            ...
            if ((compare(n1.key, k) == 0) // FIXME need to judge n1 != head &&
                            // n1 != tail?
                            && ((!delVal) || (n1.data.getReference() == dd)) && !n1.data.isMarked()) { // FIXME should compare
                // value.isMarked()
        ...
        for (i = 0; i <= n1.validLevel - 1; i++) {
            do {
                ...
                n2 = tempn1.getReference();
            } while (!tempn1.compareAndSet(n2, n2, false, true) && !tempn1.isMarked()); // FIXME isMarked should be put
            // before CAS?
        }
        ...
        for (i = n1.validLevel - 1; i >= 0; i--) {
            ...
            while (true) {
                ...
                if ((last != n1) || (n1.next.get(i).getReference() == null))
                    break;
                if (prev.next.get(i).compareAndSet(n1, n1.next.get(i).getReference(), false, false)) {
    ...
    private K fdValue(V d, boolean del) {
        ...
        n_jump: while (true) {
            ...
            if (tempn1.isMarked() && (n2 != null)) { // FIXME should be not
                ...
                v2 = n2.version;
                k2 = n2.key;
                if ((compare(n1.key, k1) == 0) && (n1.validLevel > 0) && (n1.next.get(0).getReference() == n2) && (n1.version == v) && (n2.validLevel > 0) && (n2.version == v2)) // FIXME miss node2->key == key2
                    ok = true;
            }
            if (!ok) {
                tn1 = readNext(last, 0);
