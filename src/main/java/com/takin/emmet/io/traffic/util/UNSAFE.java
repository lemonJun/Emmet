...
public abstract class UNSAFE {
    ...
    public static void assertReferenceAligned(long address) {
        if (!isReferenceAligned(address)) {
            throw new IllegalArgumentException("Memory access to object references must be " + OBJECT_REFERENCE_ALIGN + "-bytes aligned, but the address usecd was " + address);
        }
    }
    ...
    private static int getIntByByte(Object object, long offset, boolean bigEndian) {
        if (bigEndian) {
            return Bits.toInt(getByte(object, offset), getByte(object, offset + 1), getByte(object, offset + 2), getByte(object, offset + 3));
        } else {
            return Bits.toInt(getByte(object, offset + 3), getByte(object, offset + 2), getByte(object, offset + 1), getByte(object, offset));
        }
    }
    ...
    private static int getIntByByte(long address, boolean bigEndian) {
        if (bigEndian) {
            return Bits.toInt(getByte(address), getByte(address + 1), getByte(address + 2), getByte(address + 3));
        } else {
            return Bits.toInt(getByte(address + 3), getByte(address + 2), getByte(address + 1), getByte(address));
        }
    }
    ...
    private static long getLongByByte(Object object, long offset, boolean bigEndian) {
        if (bigEndian) {
            return Bits.toLong(getByte(object, offset), getByte(object, offset + 1), getByte(object, offset + 2), getByte(object, offset + 3), getByte(object, offset + 4), getByte(object, offset + 5), getByte(object, offset + 6), getByte(object, offset + 7));
        } else {
            return Bits.toLong(getByte(object, offset + 7), getByte(object, offset + 6), getByte(object, offset + 5), getByte(object, offset + 4), getByte(object, offset + 3), getByte(object, offset + 2), getByte(object, offset + 1), getByte(object, offset));
        }
    }
    ...
    private static long getLongByByte(long address, boolean bigEndian) {
        if (bigEndian) {
            return Bits.toLong(getByte(address), getByte(address + 1), getByte(address + 2), getByte(address + 3), getByte(address + 4), getByte(address + 5), getByte(address + 6), getByte(address + 7));
        } else {
            return Bits.toLong(getByte(address + 7), getByte(address + 6), getByte(address + 5), getByte(address + 4), getByte(address + 3), getByte(address + 2), getByte(address + 1), getByte(address));
        }
    }
