...
public class HashedWheelTimer implements ScheduledExecutorService {
    ...
    public ScheduledFuture<?> scheduleAtFixedRate(Runnable runnable, long initialDelay, long period, TimeUnit unit) {
        return scheduleFixedRate(TimeUnit.NANOSECONDS.convert(period, unit), TimeUnit.NANOSECONDS.convert(initialDelay, unit), constantlyNull(runnable));
    }

    @Override
    public ScheduledFuture<?> scheduleWithFixedDelay(Runnable runnable, long initialDelay, long delay, TimeUnit unit) {
        return scheduleFixedDelay(TimeUnit.NANOSECONDS.convert(delay, unit), TimeUnit.NANOSECONDS.convert(initialDelay, unit), constantlyNull(runnable));
    }
    ...
    public Runnable debounce(Runnable delegate, long period, TimeUnit timeUnit) {
        ...
        return new Runnable() {
            @Override
            public void run() {
                ScheduledFuture<?> future = reg.getAndSet(scheduleOneShot(TimeUnit.NANOSECONDS.convert(period, timeUnit), new Callable<Object>() {
                    @Override
    ...
    public <T> Consumer<T> debounce(Consumer<T> delegate, long period, TimeUnit timeUnit) {
        ...
        return new Consumer<T>() {
            ...
            public void accept(T t) {
                ScheduledFuture<T> future = reg.getAndSet(scheduleOneShot(TimeUnit.NANOSECONDS.convert(period, timeUnit), new Callable<T>() {
                    @Override
    ...
    private <V> Registration<V> scheduleFixedDelay(long recurringTimeout, long firstDelay, Callable<V> callable) {
        ...
        Registration<V> r = new FixedDelayRegistration<>(firstFireRounds, callable, recurringTimeout, rounds, offset, this::reschedule);
        wheel[idx(cursor + firstFireOffset + 1)].add(r);
        return r;
    }
