...
public class LockFreeBlockQueue<E> extends AbstractQueue<E> implements BlockingQueue<E> {
    ...
    private static final AtomicReferenceFieldUpdater<LockFreeBlockQueue, Node> tailUpdater = AtomicReferenceFieldUpdater.newUpdater(LockFreeBlockQueue.class, Node.class, "tail");
    @SuppressWarnings("unchecked")
    private static final AtomicReferenceFieldUpdater<LockFreeBlockQueue, Node> headUpdater = AtomicReferenceFieldUpdater.newUpdater(LockFreeBlockQueue.class, Node.class, "head");
                    ...
                    if (tail != head) { // more than 1 node
                        if (null != fstNodePrev) {
                            if (casHead(head, fstNodePrev)) {
                                ...
                                if (sizetmp2 < 0) {
                                    System.out.println("size = " + sizetmp1 + ":" + sizetmp2 + " , val = " + val + ", dummy = " + dummy + ", head = " + head + ", tail = " + tail + ", fstNodePrev = " + fstNodePrev);
                                }
                                if (val == null) {
