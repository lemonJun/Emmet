...
public final class GraphAlg {
     ...
     */
    public static <E> Collection<Collection<Node<E>>> getStrongComponents(DirectedGraph<E> graph, ExecutorService threadPool) {
     ...
     *         collection of graph nodes
     */
    public static <E> Collection<Collection<Node<E>>> getConnectedComponents(UndirectedGraph<E> graph, ExecutorService threadPool) {
        ConnectedComponents<E> cc = new ConnectedComponents<E>(graph, threadPool);
    ...
    private static class ConnectedComponents<E> {
        ...
        public ConnectedComponents(UndirectedGraph graph) {
            this(graph, Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors()));
        }
                    ...
                    callables[i] = new Callable<Integer>() {
                        public Integer call() {
                            ...
                            for (int j = begin; j < end; j++) {
                                ...
                                itr = graph.getLinkedNodes(nodesArray.get(j)).iterator();
                                while (itr.hasNext())
                                    minVertexValue = Math.min(minVertexValue, pvalues[map.get(itr.next().getNode())]);
        ...
        // array
        // "tasks"
        private void runTasks(Runnable[] tasks, int start, int end) throws Exception {
        ...
        // array
        // "tasks"
        private void runTasks2(Callable<Integer>[] tasks, int start, int end) throws Exception {
    ...
    private static class StrongComponents<E> {
        ...
        private BlockingQueue<HashSet<Node<E>>> workQueue = new LinkedBlockingQueue<HashSet<Node<E>>>(INIT_SIZE_FOR_WORK_QUEUE);
        ...
        private class Worker implements Runnable {
            ...
            public void run() {
                while (true) {
                    try {
                        HashSet<Node<E>> nodes = workQueue.poll(TIMEOUT, TimeUnit.MILLISECONDS);
         ...
         */
        private HashSet<Node<E>> bfs(DirectedGraph<E> dg, HashSet<Node<E>> nodes, Node<E> node) {
            // long t1 = System.nanoTime();
            // node is one of the elements of nodes, nodes is a subset of dg's
            // nodes
        ...
        private void computeSCCSeriallyTarjan(Set<Node<E>> nodes) {
            Map<Node<E>, WrappedNode> map = new HashMap<Node<E>, WrappedNode>(THRESHOLD_SERIAL);
            Iterator<Node<E>> iter = nodes.iterator();
            // int i = 0;
            ...
            while (iter.hasNext()) {
                ...
                while (!stack.isEmpty()) {
                        ...
                        if (c != null && c.size() > 0) {
                            ...
                            while (innerIter.hasNext()) {
                                ...
                                if (temp.state == WrappedNode.UNVISITED)
                                    temp.lowlink = min(temp.lowlink, innerWnode.lowlink);
                    ...
                    while (innerIter.hasNext()) {
                        ...
                        if (innerWnode != null && innerWnode.color == WrappedNode.WHITE)
     ...
     */
    public static <E> Graph<E> getMST(final UndirectedGraph graph, ExecutorService pool) throws InterruptedException, ExecutionException {
        ...
        while ((size = (nodes = edges.keySet()).size()) > 1) {
            ...
            runTasks(tasks, 0, index, pool);
            if (finish.get())
                throw new IllegalArgumentException("graph is not a connected graph");
    ...
    private static <E> Edge<E> getMinimumEdgeWith(ConcurrentHashMap<Node<E>, Collection<Edge<E>>> map, Node<E> node) {
    ...
    private static void runTasks(Runnable[] tasks, int start, int end, ExecutorService pool) throws InterruptedException, ExecutionException {
        Future<Runnable>[] futures = new Future[end - start];
     ...
     */
    public static <E> double getShortestPath(Graph<E> graph, ExecutorService exec, E source, E end) {
        // if source is the same as end, return 0
        if (source.equals(end))
            return 0;

        return getShortestPath(graph, exec, new Node<E>(source), new Node<E>(end));
    }
     ...
     * @return the length of shortest path from source node to end node
     */
    public static <E> double getShortestPath(Graph<E> graph, ExecutorService exec, Node<E> source, Node<E> end) {
        // if source is the same as end, return 0
        ...
        final CompletionService<Boolean> completionService = new ExecutorCompletionService<Boolean>(exec);
        ...
        final PriorityQueue<AdjacentNode<E>> queued = new PriorityQueue<AdjacentNode<E>>(nodeSize, nodeComparator);
         ...
         * parallely
         */
        final PriorityQueue<AdjacentNode<E>> thresholds = new PriorityQueue<AdjacentNode<E>>(nodeSize, nodeComparator);
        // enqueue the source node as initiation
     ...
     */
    private static synchronized <E> void decreasKey(PriorityQueue<AdjacentNode<E>> heap, AdjacentNode<E> node) {
        // iterator of heap
        ...
        while (iter.hasNext()) {
            ...
            if (temp.getNode().equals(node.getNode()) && temp.getWeight() > node.getWeight()) {
                // remove it from heap
                iter.remove();
    ...
    private static <E> ConcurrentHashMap<Node<E>, AdjacentNode<E>> computePredecessor(Graph<E> graph, Node<E> source) {
